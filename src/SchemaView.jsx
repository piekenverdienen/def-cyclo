...
function computeTss(blocks) {
  const score = blocks.reduce((sum, b) => {
    if (b.type === 'interval') {
      const work = b.repeats * b.minutes * Math.pow(b.factor, 2);
      const rest = b.repeats * b.rest * Math.pow(b.restFactor || 0.55, 2);
      return sum + work + rest;
    }
    return sum + b.minutes * Math.pow(b.factor, 2);
  }, 0);
...
function generateSchema({ level, days, ftp, hours }) {
  const plans = {
    beginner: {
      ...
      tempo: [
        ...
        { type: 'tempo', minutes: 20, factor: 0.8 },
        { type: 'endurance', minutes: 10, factor: 0.7 },
        { type: 'cooldown', minutes: 5, factor: 0.5 },
      ],
      steigerung: [
        { type: 'warmup', minutes: 10, factor: 0.55 },
        { type: 'interval', minutes: 5, factor: 0.85, repeats: 1, rest: 1, restFactor: 0.6 },
        { type: 'interval', minutes: 5, factor: 0.95, repeats: 1, rest: 1, restFactor: 0.6 },
        { type: 'interval', minutes: 5, factor: 1.05, repeats: 1, rest: 1, restFactor: 0.6 },
        { type: 'cooldown', minutes: 5, factor: 0.5 },
      ],
      vo2max: [
        { type: 'warmup', minutes: 10, factor: 0.6 },
    ...
    intermediate: {
      ...
      tempo: [
        ...
        { type: 'tempo', minutes: 30, factor: 0.82 },
        { type: 'endurance', minutes: 10, factor: 0.7 },
        { type: 'cooldown', minutes: 5, factor: 0.5 },
      ],
      steigerung: [
        { type: 'warmup', minutes: 10, factor: 0.6 },
        { type: 'interval', minutes: 6, factor: 0.9, repeats: 1, rest: 1, restFactor: 0.6 },
        { type: 'interval', minutes: 6, factor: 1.0, repeats: 1, rest: 1, restFactor: 0.6 },
        { type: 'interval', minutes: 6, factor: 1.1, repeats: 1, rest: 1, restFactor: 0.6 },
        { type: 'cooldown', minutes: 5, factor: 0.5 },
      ],
      vo2max: [
        { type: 'warmup', minutes: 10, factor: 0.65 },
    ...
    advanced: {
      ...
      steigerung: [
        { type: 'warmup', minutes: 10, factor: 0.65 },
        { type: 'interval', minutes: 7, factor: 1.0, repeats: 1, rest: 1, restFactor: 0.6 },
        { type: 'interval', minutes: 7, factor: 1.1, repeats: 1, rest: 1, restFactor: 0.6 },
        { type: 'interval', minutes: 7, factor: 1.2, repeats: 1, rest: 1, restFactor: 0.6 },
        { type: 'cooldown', minutes: 5, factor: 0.5 },
      ],
      vo2max: [
        { type: 'warmup', minutes: 10, factor: 0.7 },
  ...
  for (let week = 1; week <= 6; week++) {
    ...
    const weekTss = workouts.reduce((sum, w) => sum + w.tss, 0);
    weeks.push({ week: `Week ${week}`, workouts, tss: weekTss });
  }
...
export default function SchemaView({ intake, onUpdateFtp }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-700 via-purple-900 to-black p-6 text-white">
      <div className="max-w-4xl mx-auto bg-white bg-opacity-90 shadow rounded-lg p-6 space-y-6 text-gray-800">
        <h1 className="text-3xl font-bold text-center text-purple-700">Trainingsschema</h1>
        ...
        <div className="space-y-6">
          {weeks.map((week, wIndex) => (
            <div key={wIndex} className="border rounded p-4 bg-purple-50">
              <h2 className="text-xl font-semibold text-purple-800 flex justify-between">
                <span>{week.week}</span>
                <span>TSS: {week.tss}</span>
              </h2>
              <div className="grid gap-4 mt-2">
                {week.workouts.map((item, index) => (
                  <div key={index} className="bg-white p-4 rounded shadow-sm border border-purple-200">
                    <h3 className="text-lg font-medium text-purple-800 flex justify-between">
