  ...
  blocks.forEach((block) => {
    if (block.type === 'interval') {
      for (let i = 0; i < block.repeats; i++) {
        lines.push(`Interval ${block.minutes}m ${Math.round(ftp * block.factor)}W`);
        lines.push(`Rust ${block.rest}m ${Math.round(ftp * block.restFactor)}W`);
      }
    } else {
      lines.push(`${block.type} ${block.minutes}m ${Math.round(ftp * block.factor)}W`);
    }
  });
...
function downloadFit(title, blocks, ftp) {
  const data = generateFitWorkout(title, blocks, ftp);
...
export default function SchemaView({ intake, onUpdateFtp }) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-800 to-black text-white p-6">
