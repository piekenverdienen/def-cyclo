...
export default function TrainingIntake({ onComplete }) {
  ...
  const handleNext = (e) => {
    ...
    if (!email || !email.includes('@')) {
      setError('Vul een geldig e-mail adres in');
      return;
    }
    setError('');
    setStep(2);
    ...
    <div className="min-h-screen flex items-center justify-center">
      ...
      >
        ...
        {step === 2 && (
          <>
            <div>
              <label className="block text-gray-600 mb-1 flex items-center">Niveau
                <span className="ml-1 text-sm text-blue-600 cursor-pointer relative group">ℹ
                  <span className="absolute hidden group-hover:block bg-white text-gray-700 text-xs p-2 rounded shadow w-40 z-10 top-5 left-0">
                    Beginner: nieuw; Gevorderd: regelmatige fietser; Expert: zeer ervaren
                  </span>
                </span>
              </label>
              <select
